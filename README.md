# Contact Info Form & Semantic HTML
...
- Added radio buttons inside `<fieldset>` with corresponding `<legend>` for grouping related inputs
- Linked a `<label>` to a `<textarea>` using `for` and `id` attributes instead of wrapping
- Used `<details>` and `<summary>` to create collapsible content for optional user input
- Implemented color input with `type="color"` for uniform selection
- Added a text input linked to a `<datalist>` to provide selectable predefined role options
- Included agreement checkbox and submit button with proper accessibility-friendly markup
...
- Datalist, Details/Summary, Fieldset
- GitLab Pages
